...
@section('content')
                                ...
                                <div class="p-3 p-lg-5 border bg-white">
                                    ...
                                    <div class="form-group">
                                        ...
                                        @if ($stripe)
                                            ...
                                            <input type="submit" id="stripeButton"
                                                class="btn btn-black btn-lg py-3 btn-block" value="Pay with Stripe">
                                        @endif

                                        @if ($paypal)
                                            <!-- Paypal Payment Button -->
                                            <input type="submit" id="paypalButton"
                                                class="btn btn-black btn-lg py-3 btn-block" value="Pay with Paypal">
                                        @endif
    ...
    <script>
        @if (session('payment_option_error'))
            document.addEventListener('DOMContentLoaded', function() {
                let errorMessage = @json(session('payment_option_error'));
                alert(errorMessage);
            });
        @endif
        ...
        $(document).ready(function() {
            $('#stripeButton').click(function() {
                if ($('#paymentForm')[0].checkValidity()) { // Check form validity before submission
                    $('#paymentForm').attr('action',
                        '{{ route('stripecharge', ['cartItems' => $cartItems]) }}');
                    $('#paymentForm').submit();
                } else {
            ...
            $('#paypalButton').click(function() {
                if ($('#paymentForm')[0].checkValidity()) { // Check form validity before submission
                    $('#paymentForm').attr('action',
                        '{{ route('paypalcharge', ['cartItems' => $cartItems]) }}');
                    $('#paymentForm').submit();
                } else {
                    // Let HTML5 form validation handle the error message
                    $('#paymentForm')[0].reportValidity();
        ...
        @if (session()->has('errorpaypal'))
            ...
            $(document).ready(function() {
                ...
                if (existingItemIds.length > 0) {
                    ...
                    message += "Please remove this book" + (existingItemIds.length > 1 ? "s" : "") +
                        " from your cart to proceed with your current purchase.";
